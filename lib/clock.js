...
// refactoring code! Better var names, less global vars! Confusing redundant variables.
const CountTime= 30; // example min

const EndTime = 1000*60*60; 
const StartTime= (1000*60*(60-CountTime))-1000;  

let ClockOut=document.querySelector("clock");
  ...
  var datms6 = datms1 - datms2 + EndTime;
	...
	if ( sw1 == 0 ) { 
	  ...
	  var ms = dat.getMilliseconds(); 
        } else { // Start Timer with count time in minutes.
          var h=0; 
	  var m=CountTime; 
	  var s=0; 
	  var ms=0;  
      ...
      s = fuehrendeNull(s);
      ClockOut.innerHTML = m + ':' + s; // display only what is needed.
    } 
